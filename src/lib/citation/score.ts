  ...
  input_schema: {
    ...
    properties: {
      ...
      competitorsMentioned: {
        ...
        description: "Liste des concurrents cités avec leur sentiment associé.",
      },
    },
...
function buildSystemPrompt(language: "fr" | "en"): string {
  if (language === "fr") {
    return [
      ...
      "Ton rôle : déterminer si la marque cible est citée, à quelle position, avec quel sentiment, et lister les concurrents mentionnés.",
  ...
  return [
    ...
    "Your role: determine whether the target brand is mentioned, in what position, with what sentiment, and list mentioned competitors.",
...
function parseToolInput(raw: unknown): Omit<ScoringResult, "costUsd" | "durationMs" | "model"> {
  ...
  const competitorsMentioned = Array.isArray(r.competitorsMentioned)
    ? r.competitorsMentioned.flatMap((entry) => {
        if (!entry || typeof entry !== "object") return [];
        const e = entry as { name?: unknown; sentiment?: unknown };
        if (typeof e.name !== "string" || !isSentiment(e.sentiment)) return [];
        return [{ name: e.name, sentiment: e.sentiment }];
      })
    : [];
