  ...
  input_schema: {
    ...
    properties: {
      ...
      competitorsMentioned: {
        ...
        description:
          "Toutes les marques concurrentes citées dans la réponse (trackées ou non), avec leur sentiment et leur position.",
      },
    },
...
function buildSystemPrompt(language: "fr" | "en"): string {
  if (language === "fr") {
    return [
      ...
      "Ton rôle : déterminer si la marque cible est citée, à quelle position, avec quel sentiment, et lister TOUTES les marques concurrentes citées dans la réponse avec leur position et leur sentiment — y compris celles qui ne figurent pas dans la liste de concurrents fournie (elles servent à découvrir qui est recommandé à la place de la marque cible).",
  ...
  return [
    ...
    "Your role: determine whether the target brand is mentioned, in what position, with what sentiment, and list ALL competing brands mentioned in the response with their position and sentiment — including brands absent from the provided competitor list (they reveal who gets recommended instead of the target brand).",
...
function buildUserPrompt(input: ScoringInput): string {
  ...
  return [
    ...
    "",
    "Concurrents suivis (à matcher en priorité, mais liste aussi toute autre marque citée) :",
