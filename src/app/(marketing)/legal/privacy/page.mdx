...
**Pixel de mesure d'audience (pour nos clients)** : nos clients peuvent installer sur leur propre site un script first-party fourni par Mamie GEO. Ce pixel mesure deux choses : le nombre total de pages vues sur le site, et, parmi elles, celles dont le visiteur arrive depuis un moteur d'IA (ChatGPT, Perplexity, Gemini, Le Chat…). Il incrémente des compteurs quotidiens (total et par moteur). Il est **cookieless** : aucun cookie, aucun stockage local, aucun identifiant de visiteur, aucun suivi d'un visiteur d'une page à l'autre ni d'un jour à l'autre. Il compte des pages vues, pas des visiteurs uniques. Aucune adresse IP n'est conservée : l'IP n'est utilisée que de façon éphémère, sous forme de hachage salé renouvelé chaque jour, à seule fin de limiter les abus de l'endpoint, et n'est jamais stockée en base. Seuls des agrégats anonymes (nombre de pages vues par jour, et par moteur d'IA) sont conservés. Aucune donnée personnelle n'étant traitée, ce pixel ne requiert ni cookie ni bannière de consentement (directive ePrivacy).
