...
**Pixel d'attribution du trafic IA (pour nos clients)** : nos clients peuvent installer sur leur propre site un script first-party fourni par Mamie GEO. Ce pixel ne mesure qu'une seule chose : lorsqu'un visiteur arrive depuis un moteur d'IA (ChatGPT, Perplexity, Gemini, Le Chat…), il incrémente un compteur quotidien. Il est **cookieless** : aucun cookie, aucun stockage local, aucun identifiant de visiteur. Aucune adresse IP n'est conservée : l'IP n'est utilisée que de façon éphémère, sous forme de hachage salé renouvelé chaque jour, à seule fin de limiter les abus de l'endpoint, et n'est jamais stockée en base. Seuls des agrégats anonymes (nombre de visites par moteur et par jour) sont conservés. Aucune donnée personnelle n'étant traitée, ce pixel ne requiert ni cookie ni bannière de consentement (directive ePrivacy).
