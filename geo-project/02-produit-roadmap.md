...
| **3. Position par concurrent** ✅ livré 2026-06-10 | `position` (first_paragraph/middle/end) ajoutée aux `competitorsMentioned` du tool schema scoring + parsing lénient (anciens payloads sans le champ restent valides). La donnée s'accumule à chaque nouveau scoring → ranking de prééminence branchable quand l'historique sera suffisant. | **≈ 0** (même appel Haiku, +qq tokens output) | petite PR |
