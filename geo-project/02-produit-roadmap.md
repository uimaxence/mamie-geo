...
| **Vue « Prompts + analytics » + « Top Sources »** ✅ livré 2026-06-17 (cf. doc 09) | Voir d'un coup d'œil ce que chaque prompt « donne » (rang vs concurrents, visibilité, mentions, persistance, top concurrents) sans ouvrir chaque détail + classer les domaines cités comme sources par les IA. | `aggregatePromptMetrics` (pur) + `listPromptsWithMetrics` (une passe runs, zéro LLM) → toggle Liste/Analytics sur `/app/prompts` (`PromptsAnalyticsTable`) + carte « Mentions récentes » dashboard. `aggregateSourceDomains` + `TopSourcesCard` (dashboard + onglet Sources). Tags = `category`, Localisation = `language`. |
